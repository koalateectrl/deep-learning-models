...
class AnchorGenerator(nn.Module):
    ...
    def generate_anchors(self, scales: List[int], aspect_ratios: List[float], dtype: torch.dtype = torch.float32,
        ...
        hw = (h_ratios[:, None] * scales[None, :]).view(-1)

        base_anchors = torch.stack([-ws, -hw, ws, hw], dim=1) / 2
...
cell_anchors = [anchor_generator.generate_anchors(sizes, aspect_ratios) for sizes, aspect_ratios in zip(anchor_sizes, aspect_ratios)]